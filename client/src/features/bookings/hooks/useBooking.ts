...
import { getReviewByBooking } from "../../reviews/api/reviewApi";
...
export const useBooking = (userRole?: "user" | "provider") => {
  ...
  const handleStatusUpdate = useCallback(
    async (id: string, status: Booking["status"], comment?: string) => {
      try {
        ...
        dispatch(
          updateBookingStatus({ id, status, rejectionComment: comment }),
        );
  ...
  const markReviewed = useCallback((bookingId: string, rating: number) => {
